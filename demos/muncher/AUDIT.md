...
## T15 — detailed final review

- **Memory:** muncher.prg = 9,806 bytes; code+data+BSS end at $2EA6 →
  20,826 bytes free below screen RAM. Spec headroom (>=4K) exceeded 5x.
- **Frame budget:** measured, not estimated — the stress scene (4 ghosts,
  fruit, waka over siren, full HUD churn) runs 20 s with overruns=0; a live
  600-tick keyed game also measures 0.
- **Trademark scan:** grep over all sources, maps, README for the arcade
  marks and character names found one stale source comment (fixed); shipped
  strings/screens contain only the original cast.
- **Numeric tables diffed against spec:** blue times (§9), flash counts,
  house counters (§7), speed classes (§8: $40/$48/$50/$48 = 80/90/100/90%
  of $50), scaled Elroy/fruit thresholds regenerated by mazecheck.py from
  the authored dot counts. All match.
- **Known accepted divergences** (documented, hardware- or scope-driven):
  25-row maze adaptations; monochrome wall styles for palettes; hollow
  slab outlines; she waits for the first keypress instead of auto-running
  left; single-key rollover on the 4032 keyboard; no READY!/PLAYER ONE
  card (the start jingle covers the beat); popup shows 4 chars clipped to
